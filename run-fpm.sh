...
/systpl/systpl.jinja.py /systpl/fpm-php.ini.tmpl > /etc/php/7.1/fpm/php.ini
/systpl/systpl.jinja.py /systpl/php-fpm.conf.tmpl > /etc/php/7.1/fpm/php-fpm.conf
...
/systpl/systpl.jinja.py /systpl/newrelic.ini.tmpl > /etc/php/7.1/mods-available/newrelic.ini

env | awk 'BEGIN { FS="=" } /^VAR_.*=.+/ { print "env[" $1 "]='\''" $2 "'\''" }' > /etc/php/7.1/fpm/env.conf
